...
class PAnimal{
	...
	PVector first_foot_base=new PVector();
	PVector last_foot_base=new PVector();
	...
	PAnimal(float x_,float y_,float wid_,float hei_){
		...
		mleg=(int)random(3,6);
		leg_wid=body_wid/mleg*random(.15,.3);
	...
	void draw(PGraphics pg,boolean draw_fill){
		...
		if(transport_mode==3|| transport_mode==4){
			boolean change=false;//sship.draw(pg,draw_fill);
			if(change){
		...
		}else if(transport_mode==4){
			pg.translate(x+jump_pos_x-body_wid/1.2,y+jump_pos_y-body_hei*1.8);
		}else{
			pg.translate(x,y);
		}
		...
		if(transport_mode!=4) pg.translate(jump_pos_x,jump_pos_y);
		...
		// 	//water.x=cur_body_wid/4;
			...
			int stage=2;//(int)((frame_portion%TWO_PI)/(PI/2));
