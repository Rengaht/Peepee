class DistortGraphic{
	...
	DistortGraphic(int wid_seg){
		...
		pg=createGraphics((int)(width*1.2),(int)(height*1.2));
	}

	void draw(PGraphics dpg){
		// int len=vertexPoints.size();
		// dpg.background(0,0);
		dpg.pushMatrix();
		dpg.translate(-width/20,0);
		for(int i=0;i<WID_SEG-1;++i)
			for(int j=0;j<WID_SEG-1;++j){			
				...
				dpg.beginShape();
				dpg.texture(pg);
					dpg.vertex(p1.x,p1.y,t1.x,t1.y);
					dpg.vertex(p2.x,p2.y,t2.x,t2.y);
					dpg.vertex(p3.x,p3.y,t3.x,t3.y);
					dpg.vertex(p4.x,p4.y,t4.x,t4.y);
				dpg.endShape();
			}
		dpg.popMatrix();
		// update();
